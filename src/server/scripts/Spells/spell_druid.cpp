...
class spell_dru_eclipse : public AuraScript
{
    ...
    void ProcSolar(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
    ...
    void ProcLunar(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        PreventDefaultAction();
...
class spell_dru_frenzied_regeneration : public AuraScript
{
    ...
    void PeriodicTick(AuraEffect const* aurEff)
    {
        ...
        int32 const regen = CalculatePct(GetTarget()->GetMaxHealth(), GetTarget()->CalculateSpellDamage(GetCaster(), GetEffectInfo(EFFECT_1)) * mod / 100.f);
...
class spell_dru_glyph_of_innervate : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_DRUID_GLYPH_OF_INNERVATE_REGEN, GetCastDifficulty());
...
private:
    ...
    void Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        if (GetTarget()->GetShapeshiftForm() == FORM_CAT_FORM && GetTarget()->HasUnitFlag(UNIT_FLAG_STUNNED) &&
            GetTarget()->HasAuraWithMechanic(1 << MECHANIC_STUN))
            absorbAmount = CalculatePct(dmgInfo.GetDamage(), absorbPct);
    }
...
class spell_dru_savage_roar_aura : public AuraScript
{
    ...
    void AfterApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        target->CastSpell(target, SPELL_DRUID_SAVAGE_ROAR, args);
    }
